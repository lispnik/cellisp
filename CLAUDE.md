...
**Cell class & sheet struct** (`cell.lisp`, `sheet.lisp`) — `cell` is a **CLOS class** (accessors keep the `cell-` prefix: `cell-value`, `cell-precedents`, …); it caches its `value` (or an `err` condition when evaluation failed) alongside its `formula`, plus two adjacency lists: `precedents` (refs it reads) and `dependents` (refs that read it), and memoizes a `compiled` thunk with the `compiled-from` formula it was built from (see `eval-formula` below). `volatile-cell` is a subclass with no extra slots; the `volatile-p` generic dispatches on it. A `sheet` is a `defstruct` — a ref→cell hash table, an `environment` alist of `(symbol . value)` constants exposed to every formula, and a `volatiles` ref-set registry. (Note: `cell` the class and `cell` the in-formula reader function share one symbol.)
