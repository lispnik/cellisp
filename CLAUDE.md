...
- **Behavior mixins** (composable; each overrides a *different* generic, listed in `*mixin-classes*`): `observable-mixin` (overrides `cell-swept`) fires subscriber callbacks post-sweep; `readonly-mixin` (overrides `cell-writable-p`) makes the public mutators signal `readonly-cell` while still letting the cell recompute from precedents internally. Because they guard different generics, they compose on one cell (`observable-mixin+readonly-mixin+cell`). A concrete cell is one value-source class plus a *set* of mixins: `combined-class` takes `(base mixins)`, sorts the mixins by name (so any permutation memoizes to one class) and lazily defines `A+B+…+base` via `eval`+`defclass`. `cell-value-source`/`cell-mixins` decompose a cell; `morph-cell`/`add-mixin`/`remove-mixin` recombine it. So mixins layer onto *any* value source and stack with each other, `set-external`/`set-async` preserve existing mixins across a value-source change, and `unobserve` drops `observable-mixin` when the last subscriber leaves. Add a new cross-cutting axis by defining a mixin (overriding some generic) and adding it to `*mixin-classes*`.
