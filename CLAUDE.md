...
- **Observation** (composable mixin; overrides `cell-swept`): `observable-mixin` fires subscriber callbacks post-sweep. `observe` combines it with the cell's *current* value-source class on the fly via `combined-class` (memoized `eval`+`defclass`, e.g. `observable-mixin+external-cell`) and `change-class`es into it — so any cell can be observed.
