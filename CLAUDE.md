...
- **Behavior mixins** (composable, listed in `*mixin-classes*`): `observable-mixin` (primary `cell-swept`) fires subscriber callbacks post-sweep; `readonly-mixin` (overrides `cell-writable-p`) makes the public mutators signal `readonly-cell` while the cell still recomputes internally; `logged-mixin` (an `:after` on `cell-swept`, read via `cell-log`) records value history; `cached-mixin` (an `:around` on `compute-value`, toggled by `set-cached`) memoizes — it re-runs the real computation only when a precedent's value changed, re-noting precedents on a cache hit so links survive. They compose three ways: on *different* generics (readonly vs observable); on the *same* generic via method combination (`logged`'s `:after` alongside `observable`'s primary `cell-swept`); and by *wrapping* a generic (`cached`'s `:around` over whatever value source produces `compute-value`). Any subset stacks — e.g. `cached-mixin+logged-mixin+observable-mixin+readonly-mixin+cell`. A concrete cell is one value-source class plus a *set* of mixins: `combined-class` takes `(base mixins)`, sorts the mixins by name (so any permutation memoizes to one class) and lazily defines `A+B+…+base` via `eval`+`defclass`. `cell-value-source`/`cell-mixins` decompose a cell; `morph-cell`/`add-mixin`/`remove-mixin` recombine it. So mixins layer onto *any* value source and stack with each other, `set-external`/`set-async` preserve existing mixins across a value-source change, and `unobserve` drops `observable-mixin` when the last subscriber leaves. Add a new cross-cutting axis by defining a mixin (overriding some generic) and adding it to `*mixin-classes*`.
- **Volatility** (orthogonal attribute, *not* a class): membership in the sheet's `volatiles` ref-set registry, toggled by `set-volatile`, queried by `volatile-p`. Off the class hierarchy on purpose, so it layers onto any kind.
...
**Extension seams** (`eval.lisp` defines the generics; `taxonomy.lisp` the overrides) — `compute-value (cell sheet ref)` produces a cell's value (base method runs `eval-formula`); `cell-swept (cell sheet ref)` runs once per cell computed in a sweep, after the compute loop, for post-sweep notification (base method inert); `cell-writable-p (cell)` gates the public mutators (base method `t`). Add a value-source kind as a subclass with a `compute-value` method; add a cross-cutting behavior as a mixin (overriding a *different* generic, so combination classes never collide) listed in `*mixin-classes*` — rather than editing the core. `combined-class` then handles any arity of mixin over any value source.
